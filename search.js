...
const BASEURL = 'https://raw.githubusercontent.com/jimbarnesrtp/pf2/master/json/monsters-pf2-v2.json';
...
.then(resp => resp.json())
.then(function (data) {
  data.monsters.forEach((monsterObj) => {
    if (monsterObj['recallKnowledge'] === undefined){
      return;
    }
    else {
      for (key in monsterObj['recallKnowledge'][0]){
        ...
        if (monsterObj.recallKnowledge[0][key].length > 50){
          ...
          monsters.push(monsterObj.name);
          monstersData[monsterObj.name] = {
